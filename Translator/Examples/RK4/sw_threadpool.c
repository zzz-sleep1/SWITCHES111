...
void __threadpool_initialize(int __currentFunction)
{
    ...
    if(!__threads_created)
    {
        ...
        pthread_barrier_init(&barrier[__FUNCTION_2], NULL, __KERNELS);
        pthread_barrier_init(&barrier[__FUNCTION_3], NULL, __KERNELS);
        pthread_barrier_init(&barrier[__FUNCTION_4], NULL, __KERNELS);
