...
if [ "$SYSTEM" = "office" ] || [ "$SYSTEM" = "multicore" ]; then
    THREADS=4
fi
...
if [[ -f "$NSGA_PARAMS" ]]; then
    ...
    switches -s $SYSTEM $THREADS $THREADS -i $APP -t $THREADS -nsga -f $NSGA_PARAMS $PWD &
    echo "DONE"
    ...
    # Start SWITCHES process in the background with -nsga option
    echo -n "Start NSGA SWITCHES process......... "
    switches -s $SYSTEM $THREADS $THREADS -i $APP -t $THREADS -nsga $POPULATION $GENERATIONS $OBJECTIVES $OBJECTIVE_1 $OBJECTIVE_2 $MUTATION $CROSSOVER $PWD &
    echo "DONE"
...
do
    ...
    do
        ...
        # Translate child
        switches -s $SYSTEM $THREADS $THREADS -i $APP -t $THREADS -sched file "Generations/Gen$CV_GEN/g$CV_GEN-c$CV_POP" -a $AFFINITY
	    ...
	    cd $PWD
	    #echo -n "./$APP_BIN >> Results/Gen$CV_GEN/g$CV_GEN-c$CV_POP-time.out"
	    ./$APP_BIN >> Results/Gen$CV_GEN/g$CV_GEN-c$CV_POP-time.out
